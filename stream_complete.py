...
def stream():
    def generate():
        # 8 detailed insights about website analytics
        insights = """
...
def test_stream():
    ...
    try:
        response = requests.post('http://127.0.0.1:5000/stream', stream=True)
        full_content = ""
        ...
        for line in response.iter_lines():
            if line:
                ...
                if "'content':" in decoded:
                    try:
                        content = decoded.split("'content': '")[1].split("'")[0]
        ...
        print("\n\n" + "=" * 50)
        print(f"Total characters streamed: {len(full_content)}")
        print("=" * 50)
    except Exception as e:
        print(f"Error: {e}")

if __name__ == '__main__':
    threading.Thread(target=test_stream, daemon=True).start()
    app.run(debug=False)